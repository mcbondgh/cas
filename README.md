...
`java -jar target/guest-register-1.0-SNAPSHOT.jar`
...
docker build . -t guest-register:latest
```

Once the Docker image is correctly built, you can test it locally using

```
docker run -p 8080:8080 guest-register:latest
```
...
If the pods say `Container image "guest-register:latest" is not present with pull policy of Never` then you have not built your application using Docker or there is a mismatch in the name. Use `docker images ls` to see which images are available.
